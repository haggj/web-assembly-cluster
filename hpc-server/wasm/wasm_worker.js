...
self.addEventListener('message',  async function(event) {
    ...
    if (eventType === "INITIALISE") {
        let go = new Go();
        const { root, path } = eventData;
        WebAssembly.instantiateStreaming(fetch(root+path, {cache: "no-store"}), go.importObject)
            .then(result => {
                // This registers the function 'wasmFunction' in the domain of the worker
        ...
        if (typeof wasmFunction === 'function') {
            try{
                self.postMessage({
                    eventType: "RESULT",
                    eventData: await wasmFunction(...eventData),
                    eventId: eventId
                })
            } catch (e){
                self.postMessage({
            ...
            self.postMessage({
                eventType: "ERROR",
                eventData: "wasmFunction not registered!",
                eventId: eventId
            })
        }
